...
function imgtag(src, link, title, align, width) {
	...
	return src !== ""
		? atag(link, title, `<img src="${src}" ${alt} ${width} ${align} />`)
		: "";
}
...
module.exports = {
	...
	post_link: function (
		...
		if (USE_CANONICAL_URL && post.canonicalUrl) {
			return post.canonicalUrl;
		}
		if (USE_CUSTOM_BLOG_URL && BLOG_URL) {
	...
	parseDate(date) {
		...
		return `${parsedData.getDate()} ${
			months[parsedData.getMonth()]
		} ${parsedData.getFullYear()}`;
	},
};
